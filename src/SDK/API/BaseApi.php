...
class SuperSaaS_SDK_API_BaseApi
{
    ...
    protected function validateId ($value) {
        if (is_numeric($value)) {
            ...
            throw new SuperSaaS_SDK_Exception("Invalid id parameter: " . $value . ". Provide a integer value.");
        }
    }
    ...
    protected function validatePresent ($value) {
        if (empty($value)) {
            throw new SuperSaaS_SDK_Exception("Required parameter is missing.");
        } else {
            return $value;
        }
    }
    ...
    protected function validateDatetime ($value) {
        if ($value instanceof DateTime) {
            ...
            throw new SuperSaaS_SDK_Exception("Invalid datetime parameter: " . $value . ". Provide a DateTime object or formatted 'YYYY-DD-MM HH:MM:SS' string.");
        }
    }
    ...
    protected function validateOptions ($value, $options) {
        if (in_array($value, $options)) {
            ...
            throw new SuperSaaS_SDK_Exception("Invalid option parameter: " . $value . ". Must be one of " . join(', ', $options));
        }
    }
}