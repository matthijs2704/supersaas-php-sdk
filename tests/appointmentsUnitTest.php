...
class AppointmentsUnitTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testAgenda() {
        $this->assertNotNull($this->client->appointments->getList(12345, 67890));
    }
    ...
    public function testChangesSlot() {
        $from = date('Y-m-d H:i:s');
        $this->assertNotNull($this->client->appointments->changes(12345, $from, true));
    }
